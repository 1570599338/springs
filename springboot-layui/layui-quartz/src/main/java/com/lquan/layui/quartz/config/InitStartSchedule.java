package com.lquan.layui.quartz.config;

import com.alibaba.fastjson.JSONObject;

import com.lquan.layui.dao.TbQuartzJobMapper;
...
*/
	...
	public void run(String... args) throws Exception {
		...
		for (TbQuartzJob quartzJob:jobList) {
			String jobClassName=quartzJob.getJobName();
			String jobGroupName=quartzJob.getJobGroup();
			//构建job信息
			JobDetail jobDetail = JobBuilder.newJob(getClass(quartzJob.getJobClassPath()).getClass()).withIdentity(jobClassName, jobGroupName).build();
			if (StringUtils.isNotEmpty(quartzJob.getJobDataMap())) {
				JSONObject jb = JSONObject.parseObject(quartzJob.getJobDataMap());
			...
			//表达式调度构建器(即任务执行的时间)
	        CronScheduleBuilder scheduleBuilder = CronScheduleBuilder.cronSchedule(quartzJob.getJobCron());
