package com.lquan.layui.dao;


import com.lquan.layui.domain.Template;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;
import org.springframework.data.domain.Pageable;
import java.util.List;

/**
 * (Template)表数据库访问层
 *
 * @author lquan
 * @since 2022-01-05 16:45:43
 */
@Mapper 
public interface TemplateMapper {

    /**
     * 通过ID查询单条数据
     *
     * @param id 主键
     * @return 实例对象
     */
    Template queryById(Integer id);

    /**
     * 查询指定行数据
     *
     * @param template 查询条件
     * @param pageable         分页对象
     * @return 对象列表
     */
    List<Template> queryAllByLimit(Template template, @Param("pageable") Pageable pageable);

    /**
     * 统计总行数
     *
     * @param template 查询条件
     * @return 总行数
     */
    long count(Template template);

    /**
     * 新增数据
     *
     * @param template 实例对象
     * @return 影响行数
     */
    int insert(Template template);
    
        /**
     * 新增数据
     *
     * @param template 实例对象
     * @return 影响行数
     */
    int insertSelective(Template template);


    /**
     * 批量新增数据（MyBatis原生foreach方法）
     *
     * @param entities List<Template> 实例对象列表
     * @return 影响行数
     */
    int insertBatch(@Param("entities") List<Template> entities);

    /**
     * 批量新增或按主键更新数据（MyBatis原生foreach方法）
     *
     * @param entities List<Template> 实例对象列表
     * @return 影响行数
     * @throws org.springframework.jdbc.BadSqlGrammarException 入参是空List的时候会抛SQL语句错误的异常，请自行校验入参
     */
    int insertOrUpdateBatch(@Param("entities") List<Template> entities);

    /**
     * 修改数据
     *
     * @param template 实例对象
     * @return 影响行数
     */
    int update(Template template);

    /**
     * 通过主键删除数据
     *
     * @param id 主键
     * @return 影响行数
     */
    int deleteById(Integer id);


    /**
     * 通过条件查询
     * @param template
     * @return
     */
    List<Template> queryAllByBean(Template template);



}

